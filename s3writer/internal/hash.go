...
//
// For PutObject, Checksum can be used as a normal hash.Hash. When all data have been hashed, call SumPutObject to
// modify the [s3.PutObjectInput] with the expected algorithm and checksum.
//
// For multipart upload, Checksum must be used at two places. First, it must be used as a normal hash.Hash passing the
