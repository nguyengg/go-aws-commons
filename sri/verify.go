...
// Verifier extends Hash with SumAndVerify to verify the hash against a set of precomputed digests.
//
...
// Usage:
//
//	// pass to NewVerifier the expected hashes (digests) for the file.
//	v := NewVerifier("sha256-abcd", "sha256-efgh")
//	f, _ := os.Open("/path/to/file")
//	_, _ = f.WriteTo(v)
//	_ = f.Close()
//
//	if matches := v.SumAndVerify(nil); !matches {
//		// none of the digests match the file.
//	}
//
...
// NewVerifier returns a new Verifier that will match against the given set of digest candidates.
//
// The hash function of the first (primary) digest will be used as the primary hash function. Digests with unknown hash
// function are returned as the second value. If all digests are unrecognised, a nil Verifier is returned. Call
// Register if you are expecting custom hash functions.
func NewVerifier(primary string, additional ...string) (Verifier, []string) {
